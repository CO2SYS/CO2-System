...
%**************************************************************************
%
% This is version 2.1 (uploaded to GitHub by J. Sharp):
%
...
F=(p1==6);CO3(F)=PAR1(F)/1e6; % Convert from micromol/kg to mol/kg
...
F=(p2==6);CO3(F)=PAR2(F)/1e6; % Convert from micromol/kg to mol/kg
...
% So, the valid ones are: 12,13,15,16,23,25,26,35,36,56
...
if any(F)
    ...
    TCc(F)  = CalculateTCfrompHCO3  (PHic(F), CO3ic(F));
    FCic(F) = CalculatefCO2frompHCO3(PHic(F), CO3ic(F));
end
F=Icase==46 | Icase==56; % input (pCO2 or fCO2), CO3
if any(F)
...
CO3oc       = CalculateCO3fromTCpH(TCc, PHoc);
PCoc = FCoc./FugFac;
...
DATA=[TAc*1e6        TCc*1e6         PHic           PCic*1e6        FCic*1e6...
      HCO3inp*1e6    CO3ic*1e6       CO2inp*1e6     BAlkinp*1e6     OHinp*1e6...
      ...
      HCO3out*1e6    CO3oc*1e6       CO2out*1e6     BAlkout*1e6     OHout*1e6...
...
global K1 K2 KW KB KF KS KP1 KP2 KP3 KSi;
global TB TF TS TP TSi F
% ' SUB CalculateTAfrompHCO3, version 01.0, 3-18, written by Jon Sharp.
% ' Inputs: pH, CO3, K(), T()
% ' Output: TA
...
global K1 K2 F;
% ' SUB CalculateTCfrompHCO3, version 01.0, 3-18, written by Jon Sharp.
...
global K0 K1 K2 F
% ' SUB CalculatefCO2frompHCO3, version 01.0, 3-18, written by Jon Sharp.
% ' Inputs: pH, CO3, K0, K1, K2
% ' Output: fCO2
...
global K1 K2 KW KB KF KS KP1 KP2 KP3 KSi;
global TB TF TS TP TSi F
% ' SUB CalculatepHfromTACO3, version 01.0, 3-18, written by Jon Sharp.
...
% ' in seawater (pH > 6) it will be equally valid on any pH scale (H terms
% ' negligible) as long as the K Constants are on that scale.
K1F=K1(F);   K2F=K2(F);   KWF=KW(F);   KP1F=KP1(F);
KP2F=KP2(F); KP3F=KP3(F); TPF=TP(F);   TSiF=TSi(F);
KSiF=KSi(F); TBF=TB(F);   KBF=KB(F);   TSF=TS(F);
...
global K1 K2 F;
% ' SUB CalculatepHfromTCCO3, version 01.0, 3-18, written by Jon Sharp.
...
%       end
% check after sub to see if pH = missingn.
Discr = ((1./K2(F)).*(1./K2(F)) - 4.*(1./(K1(F).*K2(F))).*(1-TCi./CO3i));
H     = 0.5.*((-1./K2(F)) + sqrt(Discr))./(1./(K1(F).*K2(F)));
%       if (H <= 0)
%           pHctemp = missingn;
%       else
pHctemp = log(H)./log(0.1);
...
global K0 K1 K2 F;
% ' SUB CalculatepHfromfCO2CO3, version 01.0, 3-18, written by Jon Sharp.
...
% Outputs pH fCO2, in that order
% SUB CalculatepHfCO2fromTCCO3, version 01.0, 3-18, written by Jon Sharp.
...
global K1 K2 F
% ' SUB CalculateCO3fromTCpH, version 01.0, 3-18, written by Jon Sharp.
% ' Inputs: TC, pH, K1, K2
% ' Output: CO3
% ' This calculates CO3 from TC and pH, using K1, and K2.
H            = 10.^(-pHx);
...
varargout{1} = HCO3; varargout{2} = BAlk;  varargout{3} = OH; varargout{4} = PAlk;
varargout{5} = SiAlk; varargout{6} = Hfree; varargout{7} = HSO4; varargout{8} = HF;
end % end nested function
